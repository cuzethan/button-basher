...
class UpgradeSection(arcade.Section):
    ...
    def on_draw(self):
        ...
        # AutoClicker Upgrade
        arcade.draw_text(self.auto_clicker.getDesc(), self.left + 20, self.top - 100, self.auto_clicker.getColor(), 16)

        # DoubleClicker Upgrade
        arcade.draw_text(self.double_clicker.getDesc(), self.left + 20, self.top - 140, self.double_clicker.getColor(), 16)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if (self.left + 20 < x < self.left + 180 and self.top - 110 < y < self.top - 90):
            if self.game_view.score >= self.auto_clicker.cost and not self.auto_clicker.active:
                # Deduct cost and activate AutoClicker
                self.game_view.score -= self.auto_clicker.cost
                self.auto_clicker.active = True
        ...
        if (self.left + 20 < x < self.left + 180 and self.top - 150 < y < self.top - 130):
            if self.game_view.score >= self.double_clicker.cost and not self.double_clicker.active:
                # Deduct cost and activate DoubleClicker
                self.game_view.score -= self.double_clicker.cost
                self.double_clicker.active = True
